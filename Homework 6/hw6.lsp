...
(defun at-most-one-color (n c k)
    (cond ((= c k) nil)
        (t (append (pairs-between n c (+ c 1) k) (at-most-one-color n (+ c 1) k))))
  )

;Helper fuction for at-most-one-color
;This function returns a list of 2 variable clauses (-a -b), where a is the variable corresponding to (node2var n start k), and b is in the range current<=b<=k.
;Example: (pairs-between 1 3 4 6) returns ((-3 -4) (-3 -5) (-3 -6))
;Arguments: n is the node, start is the color index corresponding to the first variable in each clause, current is the current color index corresponding to the second variable in each clause, and k is the maximum color index.
(defun pairs-between (n start current k)
    (cond ((> current k) nil)
        (t (cons (append (list (* (node2var n start k) -1))(list (* (node2var n current k) -1))) (pairs-between n start (+ current 1) k)))
    ))
...
(defun generate-edge-clauses (e k)
    (generate-edge-clauses-2 e 1 k)
  )

;Helper function for generate-edge-clauses
;For all colors, the function finds variables a and b correponding to both the nodes on the edge having that color, and creates the clause (-a -b) - meaning both nodes can not have the same color.
;Arguments: e is a list of two node indices on an edge, current is the current color index, and k is the maximum color index.
(defun generate-edge-clauses-2 (e current k)
    (cond ((> current k) nil)
        (t (cons (append (list (* (node2var (first e) current k) -1))(list (* (node2var (second e) current k) -1)))(generate-edge-clauses-2 e (+ current 1) k)))  
))
